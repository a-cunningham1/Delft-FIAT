...
Instead they are interpreted as subdatasets (!). One can set the key `var_as_band` to `true` within the [settings file](settings.qmd)
...
the `subset` variable within the [settings file](settings.qmd) should be set under the header `hazard.settings`.
